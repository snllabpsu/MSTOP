classdef GenerateVariables
    ...
    properties
       %%[props1] %variables that are being pregenrated by the KINARM 
       targetY 
       targetydot
       gazetimestamp 
       gazeY 
       ...
       %%create new [props2] %variables being calculated using the
       %pregenerated code
       ...
       idx_force5perc %time at which the force is 5 % of the peak force 
       maxforce_beforecollision % peak force before collision
       idx_forceonset %index of the 5% onset of force
       forceonset %force at the index of force onset
       collision_point %index at which the collision takes place (as detected using event codes in KINARM)
       idx_forceonsetwrtcollision %index of the force onset with respect to collision. i.e after chpping the trial with respect to the chopping criteria 
       DTC% distance to collision when the force onset happened
       polyfit % slope of force increase from the time of force onset to the maximum force before collision 
       TTC %TOFO. the distance is calculated at the time of force onset between the hand and the object. and then divided by the velocity to get TOFO
...
%        gazegain_slope_mid
       ...
       gazegain_whole
       slopegg_weighted
       CPF%compensatory peak force. the peak force fromm collision to end of collision 
       
    end

    methods
        
    function s = GenerateVariables (obj)
   %obj is the condition(exp:D1_M3);
        ...
        props1(contains(props1,{'success','DeltaImpulse','gazeangvelocity','idx_force5perc','maxforce_beforecollision','idx_forceonset','forceonset','collision_point',...
            ...
            ,'ydot_plot','gazelag_stat','gazelag_plot','gazegain_stat','gazegain_plot','psperc','saccadeperc','gazeydot_BFOC','gazegain_BFOC','gazegain_middle','slope_gg','DOMO','TOMO','gazegain_whole','slopegg_weighted','CPF'})) = [];
        props2 = {'success', 'DeltaImpulse','gazeangvelocity','idx_force5perc','maxforce_beforecollision','idx_forceonset','forceonset','collision_point','idx_forceonsetwrtcollision','DTC','polyfit','TTC','pursuitperc_TTC',...
            ...
            'gazegain_stat','gazegain_plot','psperc','saccadeperc','gazeydot_BFOC','gazegain_BFOC','gazegain_middle','slope_gg','DOMO','TOMO','gazegain_whole','slopegg_weighted','CPF'};
   %preallocation props2
        ...
        s.(props2{29})= nan(2000, size(obj,3)); %gazegain_middle
        s.(props2{30})= nan(901, size(obj,3)); %gaze lag 251 timepoints to be used for plotting
        s.(props2{31})= nan(201, size(obj,3)); %gaze gain 200 ms before FOC to be used for statistical analysis
        s.(props2{32})= nan(901, size(obj,3)); %gaze gain 
        ...
        s.(props2{39}) = nan(1, size(obj,3)); %DOMO same as DTC but for hand velocity
        s.(props2{40}) = nan(1, size(obj,3)); %TOMO
        s.(props2{41}) = nan(size(obj,1), size(obj,3)); %gazegain whole
        s.(props2{42})= nan(1, size(obj,3)); %weighted slope of gazegain per trial
        s.(props2{43})= nan(1, size(obj,3)); %%% CPF compensatory peak force
        
        for iprop = 1:length(props1)
            thisprop = props1{iprop}; 
  %preallocation props1
            %s.(thisprop) = nan(size(obj,1),size(obj,3));
  ...
  %at each iteration (i) access trial(i)
              ...
              time_window_gaze=[-400 -150];%chopping criteria for gaze
           ...
           % s.(thisprop)(:,i) = Trial(obj,i).(thisprop);  % doing this to get the whole gazw event
            ...
            collsiontime=90; %make this 160ms for EIFANG3 and 90for E1 and 2
                        
            [s.(props2{43})(:,i),idxpeakforceaftercollision]= max(Trial(obj,i).handforceY(index:index+collsiontime));
            [peakvel,idxpeakvel]= max(Trial(obj,i).handYvel(handForceStartPoint:index));
            maxForceInTrial=max(Trial(obj,i).handforceY(handForceStartPoint:index));
            maxVelInTrial=max(Trial(obj,i).handYvel(handForceStartPoint:index));
            maxHandForcePoint=handForceStartPoint+idxpeakforce;
            maxHandVelPoint=handForceStartPoint+idxpeakvel;
            searchVectorHandForceBegin=Trial(obj,i).handforceY(1:maxHandForcePoint)- mean(Trial(obj,i).handforceY(1:100));
            searchVectorHandVelBegin=Trial(obj,i).handYvel(1:maxHandVelPoint)- mean(Trial(obj,i).handYvel(1:100));
            fivePercentPeakForce=0.05*(maxForceInTrial- mean(Trial(obj,i).handforceY(1:100)));
            fivePercentPeakVelocity=0.05*(maxVelInTrial- mean(Trial(obj,i).handYvel(1:100)));
            ...
            beginVelStart= find(searchVectorHandVelBegin>fivePercentPeakVelocity==0,1,'last');
...
%             forceY=Trial(obj,i).handforceY(index+time_window(1):index+time_window(2));
           ...
           elseif forceminima_after5perc < force_at5perc
               ...
               if Trial(obj,i).protocolvec(1,1)==1 | Trial(obj,i).protocolvec(1,1)==2 | Trial(obj,i).protocolvec(1,1)==3 
               ...
               elseif Trial(obj,i).protocolvec(1,1)==4
                  ...
                  s.(props2{18})(:,i)=(Trial(obj,i).handYvel(index)).*0.1;
                    ...
                    %% Now calculate TOMO and DOMO
                s.(props2{39})(:,i)=(Trial(obj,i).targetY(beginVelStart)-Trial(obj,i).handY(beginVelStart)).*0.1;
               if Trial(obj,i).protocolvec(1,1)==1 | Trial(obj,i).protocolvec(1,1)==2 | Trial(obj,i).protocolvec(1,1)==3 
                   s.(props2{40})(:,i)=(((s.(props2{39})(:,i)).*0.01)./0.25).*1000; 
               elseif Trial(obj,i).protocolvec(1,1)==5
                   s.(props2{40})(:,i)=(((s.(props2{39})(:,i)).*0.01)./0.35).*1000; 
               elseif Trial(obj,i).protocolvec(1,1)==4
                   s.(props2{40})(:,i)=(((s.(props2{39})(:,i)).*0.01)./0.15).*1000;
               end
                  
            %% Calculating the gaze variables
              ...
              if (s.(props2{6})(:,i))> 150
                  gazeangvel_BFOC=s.(props2{3})((s.(props2{6})(:,i)-150):s.(props2{6})(:,i)); 
              else
                  gazeangvel_BFOC=0;
              end
             ...
             pursuitbeforeC=s.(props2{3})((150:(s.(props2{6})(:,i)-150)),i);
             ...
             pursuitpercentage=sum(~isnan(pursuitbeforeC)/size(pursuitbeforeC,1)).*100;
             s.(props2{33})(:,i)=pursuitpercentage;
               ...
               if pursuitpercentage >= 50 && (s.(props2{9})(:,i)<=500) && (s.(props2{6})(:,i)>=351) %% considering FOC that has only occured not more than 500 ms before collision 
                  ...
                  gazegain = (s.(props2{26})(:,i))./((Trial(obj,i).targetydot).*0.1);%gaze gain for the whole trial but only for trials with 200ms before FOC and 50% pursuit
                  ...
                  gazelag_middle = gazelag(150:(s.(props2{6})(:,i)-150));
                  if (sum(~isnan(gazelag_middle))/size(gazelag_middle,1)*100) >50
                      s.(props2{29})((1:length(gazelag_middle)),i)=(gazelag_middle);
                  else
                      s.(props2{29})(:,i)=nan;
                  end
                  ...
                  s.(props2{41})(:,i)=gazegain;

                  %Finding slope of gaze gain for 150 ms before Force onset

                  timeVectorforRegressiongaze=(150:(s.(props2{6})(:,i)-150))/1000; 
                  gazegainVectorforRegression=movmean(gazegain(150:(s.(props2{6})(:,i)-150)),20,'omitnan');
                  timeVectorforRegressiongaze=timeVectorforRegressiongaze-timeVectorforRegressiongaze(1); 
                  ...
                  p_gaze = polyfit(timeVectorforRegressiongaze(~nanidx),gazegainVectorforRegression(~nanidx),1);  %only consider the values in the regression vector which are not nans.
                   ...
                   % finding slope for each chunk of gazegain and then
                   % taking the weighted mean of all the slope values.


                    idx_nan = all(isnan(gazegainVectorforRegression),2); %put 1 where there is nan in the vector
                    idr = diff(find([1;diff(idx_nan);1])); %find the last index where there is nonnan in the vector(can be multiple of them)
                    chunks_gaze_cell = mat2cell(gazegainVectorforRegression,idr(:),size(gazegainVectorforRegression,2)); %make a cell, each cell would have gazegain values, from start till a nan value was reached and then all nans, followed by values again.
                    number_chunks=size(chunks_gaze_cell,1);% find the number of chunks that has been created
                    z=0; %keep it at 0
                    for k= 1:number_chunks %loop runs for the number of chunks that has been generated
                        
                        if isempty(find(all(isnan(chunks_gaze_cell{k}),2))) && idr(k)>40 % if the chunk is full of only nans, or the chunk size is less than 40 time points then they are not considered.
                           z=z+1; %this only executes when we are looking at a chunk of interest
                           timevectorregressgaze_chunk=linspace(0,(idr(k)-1)*0.0010,idr(k)); %make the time vector considering that the sampling frequncy is 500 and therefore the time vector starts from 0 to (idr(k)-1)*0.0001 (value of time point - put breakpoint and look at the timevector formation in hand slope
                           p_gaze_chunks= polyfit(timevectorregressgaze_chunk,chunks_gaze_cell{k},1); %get slope value
                           p_gaze_chunks_store(z,1)=p_gaze_chunks(1);%store slope values for the chunks that had values (no nans)
                           multiply_slopetime(z,1)= p_gaze_chunks(1)*idr(k); %multiply the slope with the total time for which the slope happened
                           time_slopes(z)=idr(k); %get all the time values
                        end


                    end
                    if z~=0 %for situations when there is no gaze gain at all for the time interval
                   slope_time_sum= sum(multiply_slopetime); %find the summation
                   time_sum=sum(time_slopes);
                   weighted_slope_gazegain=slope_time_sum/time_sum;  %get the weighted mean for the slope gaze gain                   
                   s.(props2{42})(:,i)=weighted_slope_gazegain;
                    else
                        s.(props2{42})(:,i)=nan;
